...
// Oral presentation: The formula is already given up there and the aReal, aImag, bReal, BImage are inputs for like a, b, c,and d.
// aReal, aImag represent the first complex number (a + bi) and bReal, bImag represent the second complex number (c + di)
    ...
    // the formulate is already given like the last one. real² + imag² so pretty easy to understand.
...
// Formula's are already given. For the iter we start off with zero so it's equal to zero odviously.
// Gotta add the iter ++ to count how many times.
// Inside the loop we need to compute and do add the new z's and firt number is 0 cause arrays in Java are 0 inedex 
// So first position is 0
// Then do the escape stuff so requires then just make the if. 
    ...
    // We already have the formula. You just need to put the double to force decimal results.
    ...
    //So for the if the assigment it said iterations == MAX_ITERATIONS (200) and return and the second part is already there.
